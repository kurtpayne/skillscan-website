...
| **13f ✅** | **Security explainer: What are skills and where does SkillScan fit?** ✅ Published 2026-03-29 at `/blog/skills-security-model`. |
| **13g** | **Trace-B6 — Multi-skill trace (batch mode, max 50 files)** | `skillscan-trace` currently traces one skill file at a time. Add a batch mode that accepts a directory or glob pattern and traces up to 50 files in a single run, with a configurable `--max-skills` cap (default: 50). The fixture harness must detect when a skill load request arrives (i.e., when the traced model attempts to read or invoke a skill by name or path) and respond with the appropriate fixture content — enabling realistic multi-skill interaction patterns (skill A invokes skill B, skill B invokes skill C) to be traced without a live agent runtime. Output: per-skill verdict table + aggregate summary. Effort: ~4 hours. Kurt requested 2026-03-29. |
