...
| **13f ✅** | **Security explainer: What are skills and where does SkillScan fit?** ✅ Published 2026-03-29 at `/blog/skills-security-model`. | A long-form technical blog post (or whitepaper) aimed at security engineers and enterprise architects who have heard of "AI agent skills" but don't fully understand the threat model. Structure: (1) What skills are — runbooks for agentic consumption, not code, but sometimes shipping with code; how they differ from plugins, tools, and prompts; why the distinction matters for security. (2) The attack surface — five real attack archetypes with sanitized examples: README-driven dropper (AMOS/NemoClaw pattern), indirect injection via calendar event descriptions, hallucination squatting, telemetry exfiltration disguised as analytics, and goal substitution via trusted data channels. (3) Why static analysis can catch these — show the actual rule or ML finding for each example; demonstrate that the attack is detectable before runtime. (4) Where SkillScan fits in the broader security stack — what it covers, what requires dynamic analysis (skillscan-trace), and what requires infrastructure controls (NemoClaw-style egress blocking, VirusTotal, Cisco Umbrella, or similar). (5) Next steps for enterprise teams — recommended scanning posture, CI/CD integration, and the offline-first trust model. Publish on the website as a `/blog/skills-security-model` page and cross-post to HuggingFace model card and GitHub README. Effort: ~4 hours. Kurt requested 2026-03-29. |
