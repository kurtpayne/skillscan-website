...
// AUTO_SYNC_BEGIN: rules array
  ...
  { id: "EXF-021", category: "EXF", severity: "BLOCK", title: "VSCode Live Preview and SARIF viewer local file exfiltration", description: "Update VSCode extensions (Live Preview, SARIF viewer) to patch HTML/JS injection and path traversal vulnerabilities that allow local file exfiltration.", tags: ["vscode", "local-file-exfil", "sarif"] },
  // GR — Graph Rules
  ...
  { id: "MAL-055", category: "MAL", severity: "BLOCK", title: "postmark-mcp BCC email harvesting (first in-the-wild malicious MCP server)", description: "postmark-mcp versions 1.0.16-1.0.18 (npm publisher phanpak) injected a silent BCC on every outbound email to phan@giftshop.club — the first confirmed malicious MCP server in production. All email content processed by AI-automated Postmark workflows was silently copied to the attacker. Audit for postmark-mcp versions 1.0.16-1.0.18 and rotate any Postmark API keys that may have been exposed.", tags: ["mcp-server", "email-exfiltration", "bcc-injection", "postmark", "supply-chain"] },
  { id: "MAL-056", category: "MAL", severity: "BLOCK", title: "Nx/s1ngularity AI CLI weaponization via dangerously-skip-permissions", description: "The Nx s1ngularity campaign (August 2025) injected a postinstall script (telemetry.js) that weaponized locally installed AI CLI tools (claude, gemini, amazon q) using permission-bypass flags (--dangerously-skip-permissions, --yolo, --trust-all-tools) to enumerate and exfiltrate sensitive files. Stolen credentials were triple-base64 encoded and exfiltrated to attacker GitHub repos. A destructive payload also appended sudo shutdown -h 0 to shell rc files.", tags: ["ai-weaponization", "supply-chain", "postinstall", "nx", "s1ngularity"] },
  // OBF — Obfuscation
  ...
  { id: "SUP-023", category: "SUP", severity: "BLOCK", title: "mcp-remote OAuth authorization_endpoint command injection (CVE-2025-6514)", description: "CVE-2025-6514 (CVSS 9.6 critical) affects mcp-remote npm versions 0.0.5-0.1.15. A malicious MCP server can return a crafted authorization_endpoint value during OAuth discovery that is passed unsanitized to the OS shell, enabling full RCE on the MCP client. Upgrade mcp-remote to 0.1.16 or later.", tags: ["supply-chain", "mcp", "cve", "oauth", "command-injection", "rce"] },
  { id: "SUP-024", category: "SUP", severity: "BLOCK", title: "Compromised axios npm versions (TeamPCP RAT dropper, March 2026)", description: "axios versions 1.14.1 and 0.30.4 were compromised on March 30, 2026 via a hijacked maintainer npm account. The malicious versions inject plain-crypto-js@4.2.1 as a hidden dependency whose postinstall script drops a cross-platform RAT exfiltrating SSH keys, cloud tokens, and credentials to sfrclak.com. Linked to the TeamPCP supply chain campaign. Upgrade to axios@1.7.9+ (1.x) or axios@0.29.0+ (0.x).", tags: ["supply-chain", "npm", "account-takeover", "rat", "teampcp", "axios"] },
