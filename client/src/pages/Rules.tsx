...
const rules: Rule[] = [
  ...
  { id: "SUP-015", category: "SUP", severity: "BLOCK", title: "GitHub Actions Supply Chain Compromise via Release Tag Repointing", description: "Detects the trivy-action supply chain compromise pattern where release tags are repointed to malicious commits containing credential stealers in entrypoint.sh. Includes detection of forced tag updates and the known malicious SHA256 hash.", tags: ["supply-chain", "github-actions", "tag-repointing", "credential-theft", "trivy"] },
  { id: "MAL-044", category: "MAL", severity: "BLOCK", title: "SQLBot Stored Prompt Injection to RCE via COPY TO PROGRAM", description: "Detects CVE-2026-32622, a stored prompt injection vulnerability in SQLBot where malicious Excel files with embedded prompt injection payloads cause the AI SQL agent to execute arbitrary system commands as the postgres user via PostgreSQL COPY TO PROGRAM.", tags: ["prompt-injection", "rce", "sql", "postgres", "ai-agent", "cve"] },
  { id: "PINJ-006", category: "PINJ", severity: "BLOCK", title: "RAG Poisoning Multi-Stage AI Agent Attack Chain", description: "Detects RAG poisoning attacks where malicious content injected into vector databases manipulates AI agent tool invocations to steal Kubernetes service account tokens and exfiltrate sensitive data through manipulated retrieval-augmented generation.", tags: ["rag", "prompt-injection", "ai-agent", "knowledge-base", "data-poisoning"] },
  { id: "MAL-045", category: "MAL", severity: "BLOCK", title: "StoatWaffle Node.js Malware Family (WaterPlum/Contagious Interview)", description: "Detects the StoatWaffle Node.js malware family deployed by the WaterPlum threat actor (Contagious Interview campaign) via trojanized VS Code extensions. Identifies the multi-stage infection chain including BeaverTail dropper, InvisibleFerret RAT, and StoatWaffle's distinctive HTTP/WebSocket C2 protocol with UUID-based beaconing.", tags: ["malware", "stoatwaffle", "waterplum", "vscode", "supply-chain", "rat"] },
  { id: "SUP-016", category: "SUP", severity: "BLOCK", title: "Vulnerable MCP Server Package with Command Injection", description: "Detects vulnerable MCP server packages with known command injection or RCE vulnerabilities: CVE-2026-4198 (mcp-server-auto-commit), CVE-2026-4192 (quip-mcp-server), and CVE-2026-33252 (MCP Go SDK CSRF tool execution).", tags: ["supply-chain", "mcp", "command-injection", "rce", "cve"] },
  { id: "MAL-048", category: "MAL", severity: "BLOCK", title: "Langflow Unauthenticated RCE via build_public_tmp Endpoint", description: "Detects CVE-2026-33017, a critical unauthenticated remote code execution vulnerability in Langflow where attackers exploit the /api/v1/build_public_tmp endpoint to inject arbitrary Python code, achieving full server compromise within hours of initial access.", tags: ["langflow", "rce", "cve", "ai-pipeline", "unauthenticated"] },
  { id: "SUP-017", category: "SUP", severity: "BLOCK", title: "Checkmarx GitHub Actions Supply Chain Compromise (TeamPCP)", description: "Detects CVE-2026-33634, the TeamPCP supply chain compromise of Checkmarx GitHub Actions (ast-github-action, kics-github-action) where attackers force-pushed malicious commits to all release tags, exfiltrating SSH keys and cloud credentials to checkmarx.zone.", tags: ["supply-chain", "github-actions", "checkmarx", "teampcp", "credential-theft"] },
  { id: "MAL-049", category: "MAL", severity: "BLOCK", title: "LiteLLM .pth File Persistence and Sysmon Backdoor (TeamPCP)", description: "Detects the TeamPCP supply chain compromise of LiteLLM PyPI packages (1.82.7, 1.82.8) via a poisoned Trivy scanner. The malware injects litellm_init.pth into site-packages for Python startup persistence, creates ~/.config/sysmon/sysmon.py and a systemd user service for long-term backdoor access, and exfiltrates credentials to models.litellm.cloud.", tags: ["malware", "litellm", "teampcp", "supply-chain", "persistence", "backdoor"] },
  { id: "SUP-019", category: "SUP", severity: "BLOCK", title: "Compromised LiteLLM Package Version Reference", description: "Detects pip/poetry/pipenv install commands referencing known-malicious LiteLLM versions 1.82.7 and 1.82.8, which were compromised by TeamPCP via a poisoned Trivy scanner and contain a multi-stage credential stealer.", tags: ["supply-chain", "litellm", "teampcp", "pypi", "compromised-package"] },
  { id: "SUP-020", category: "SUP", severity: "BLOCK", title: "ClawHavoc Malicious ClawHub Skill Typosquat Names", description: "Detects known-malicious ClawHub skill names from the ClawHavoc campaign that planted 1,184+ malicious skills on ClawHub. These typosquats (clawhub1, clawhubb, solana-wallet-tracker, polymarket-trader, etc.) deliver Atomic Stealer on macOS, keyloggers on Windows, or reverse shell backdoors.", tags: ["supply-chain", "clawhavoc", "typosquat", "openclaw", "clawhub"] },
  { id: "PINJ-015", category: "PINJ", severity: "BLOCK", title: "Prompt Poaching via Malicious Browser Extension", description: "Detects instructions to install browser extensions that intercept, capture, or relay prompts sent to AI assistants. The prompt poaching technique uses malicious extensions to steal user prompts containing sensitive code, credentials, and business logic.", tags: ["prompt-injection", "browser-extension", "prompt-poaching", "data-theft"] },
  // INJ — Injection
