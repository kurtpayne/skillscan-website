...
// AUTO_SYNC_BEGIN: rules array
  ...
  { id: "MAL-062", category: "MAL", severity: "BLOCK", title: "ClawHub malicious skill keylogger/Atomic Stealer payload delivery", description: "ClawHub (OpenClaw's public skill marketplace) was found to contain 335 malicious skills in April 2026, representing approximately 12% of the registry. These skills use professional documentation and innocuous names (e.g., \"solana-wallet-tracker\") to appear legitimate, then instruct users to run external code that installs keyloggers on Windows or Atomic Stealer malware on macOS. Compromised agents can access sensitive data and OAuth tokens. Remove any skill referencing these patterns and audit all installed ClawHub skills. Report to your security team immediately.", tags: ["malware", "clawhub", "openclaw", "keylogger", "atomic-stealer", "supply-chain", "skill-registry"] },
  { id: "MAL-064", category: "MAL", severity: "BLOCK", title: "velora-dex/sdk@9.4.1 npm Go RAT dropper (minirat via alibaba.xyz)", description: "@velora-dex/sdk version 9.4.1 was compromised on April 7, 2026. Three lines injected into dist/index.js decode a base64 payload and fetch a shell script from 89.36.224.5 that downloads a compiled Go RAT (module path alibaba.xyz/minirat) and persists it via launchctl under the label zsh.profiler / com.apple.Terminal.profiler. The RAT communicates with C2 servers datahub.ink, cloud-sync.online, and byte-io.us for command execution and file exfiltration. Remove @velora-dex/sdk@9.4.1 immediately, rotate all credentials, and check for the profiler binary in ~/Library/Application Support/com.apple.Terminal/.", tags: ["malware", "npm", "supply-chain", "rat", "go-rat", "minirat", "velora-dex", "macos", "launchctl"] },
  { id: "MAL-065", category: "MAL", severity: "BLOCK", title: "hermes-px PyPI AI conversation stealer (prompt exfiltration via Supabase)", description: "hermes-px (JFROG XRAY-961094, April 5, 2026) masquerades as a privacy-first AI inference proxy with Tor routing. Instead of Tor, it proxies all requests through a hijacked Tunisian university API (prod.universitecentrale.net:9443) and exfiltrates every prompt and response to a Supabase endpoint. The package also embeds a stolen 246K-character Claude Code system prompt. Remove hermes-px immediately, rotate any API keys used with it, and audit for data leakage of sensitive prompts.", tags: ["malware", "pypi", "supply-chain", "exfiltration", "prompt-theft", "ai-proxy", "supabase", "hermes-px"] },
  { id: "MAL-063", category: "MAL", severity: "BLOCK", title: "Lazarus Group fake-recruitment npm/PyPI RAT campaign (graphalgo/bigmathutils/duer-js)", description: "Lazarus Group (DPRK/Sapphire Sleet) is targeting cryptocurrency and AI developers with fake technical interview packages on npm and PyPI. Packages including graphalgo, bigmathutils, duer-js, and xpack-utils deliver GolangGhost or BeaverTail RAT via postinstall hooks. Victims are recruited via LinkedIn with promises of high-paying roles and asked to clone a private repository and install dependencies as part of a \"coding challenge\". The postinstall hook contacts C2 infrastructure (codepool.cloud, aurevian.cloud) to download and execute a second-stage payload. Do not install packages from untrusted sources as part of recruitment processes. Verify package provenance before installation.", tags: ["malware", "lazarus", "dprk", "sapphire-sleet", "npm", "pypi", "rat", "fake-recruitment", "supply-chain", "beavertail", "golangghost"] },
  // OBF — Obfuscation
  ...
  { id: "PINJ-019", category: "PINJ", severity: "BLOCK", title: "Claudy Day invisible URL parameter prompt injection + Files API exfiltration", description: "The \"Claudy Day\" attack (demonstrated by Oasis Security, March 2026) chains three vulnerabilities against claude.ai: (1) invisible prompt injection via URL parameters containing hidden HTML tags processed by Claude but invisible to the user, (2) data exfiltration through the Anthropic Files API allowed by default in the sandbox, and (3) conversation history theft. This enables attackers to steal sensitive information (business strategy, financial data, personal details) from a default claude.ai session without requiring MCP servers or special configuration. Treat all URL-prefilled chat content as untrusted. Restrict Files API access in production deployments.", tags: ["prompt-injection", "claude", "anthropic", "files-api", "exfiltration", "claudy-day"] },
  { id: "PINJ-020", category: "PINJ", severity: "BLOCK", title: "MINJA cross-session AI agent memory poisoning via untrusted document processing", description: "MINJA (Memory INJection Attack) is a class of prompt injection attack where malicious instructions embedded in documents processed by an AI agent persist in the agent's memory store and affect future sessions with other users. Unlike single-session injection, MINJA payloads survive across conversation resets and can silently manipulate all subsequent agent interactions. Attackers embed hidden directives (e.g., in HTML comments or after legitimate content) that instruct the agent to store malicious system-level instructions in memory. Sanitize all external content before memory storage. Implement memory content filtering and audit memory stores for injected system directives.", tags: ["prompt-injection", "memory-poisoning", "cross-session", "minja", "ai-agent", "persistence"] },
  ...
  { id: "SUP-030", category: "SUP", severity: "BLOCK", title: "a11y-mcp SSRF vulnerability (CVE-2026-5323)", description: "CVE-2026-5323 is a Server-Side Request Forgery (SSRF) vulnerability in priyankark/a11y-mcp up to version 1.0.5. The A11yServer function in src/index.js allows manipulation of URL parameters to trigger unauthorized server-side requests, potentially leading to data exfiltration, service enumeration, or abuse of cloud metadata endpoints (e.g., 169.254.169.254). Upgrade to a patched version and restrict outbound network access from MCP server processes.", tags: ["supply-chain", "mcp", "ssrf", "cve-2026-5323", "a11y"] },
  { id: "SUP-032", category: "SUP", severity: "BLOCK", title: "Compromised @velora-dex/sdk@9.4.1 package version reference", description: "@velora-dex/sdk version 9.4.1 is a known-malicious npm package (compromised April 7, 2026) that delivers the minirat Go RAT. Any direct reference to this version in package.json, install commands, or lockfiles indicates a compromised dependency. Remove immediately and pin to a safe version after verifying the package registry has been cleaned.", tags: ["supply-chain", "npm", "velora-dex", "compromised-package", "rat", "minirat"] },
  { id: "SUP-031", category: "SUP", severity: "BLOCK", title: "PackageGate pnpm/npm lifecycle script bypass (CVE-2025-69264 / CVE-2025-69263)", description: "CVE-2025-69264 (pnpm < 9.15.4) and CVE-2025-69263 (npm < 11.2.0) are zero-day vulnerabilities in the PackageGate lifecycle script enforcement mechanism. Despite configuring ignore-scripts=true, malicious packages can execute preinstall and prepare lifecycle hooks by exploiting a bypass in the script-runner logic. Attackers can deliver arbitrary code execution during package installation even in environments that believe they have disabled lifecycle scripts. Upgrade pnpm to 9.15.4+ or npm to 11.2.0+. Audit installed packages for suspicious preinstall/prepare hooks. Do not rely solely on ignore-scripts as a security control.", tags: ["supply-chain", "npm", "pnpm", "lifecycle-script", "packagegate", "cve-2025-69264", "cve-2025-69263", "zero-day"] },
