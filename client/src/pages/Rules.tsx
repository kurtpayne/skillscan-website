...
// AUTO_SYNC_BEGIN: rules array
  ...
  { id: "MAL-062", category: "MAL", severity: "BLOCK", title: "ClawHub malicious skill keylogger/Atomic Stealer payload delivery", description: "ClawHub malicious skill keylogger/Atomic Stealer payload delivery", tags: ["malware", "clawhub", "openclaw", "keylogger", "atomic-stealer", "supply-chain", "skill-registry"] },
  ...
  { id: "PINJ-019", category: "PINJ", severity: "BLOCK", title: "Claudy Day invisible URL parameter prompt injection + Files API exfiltration", description: "Claudy Day invisible URL parameter prompt injection + Files API exfiltration", tags: ["prompt-injection", "claude", "anthropic", "files-api", "exfiltration", "claudy-day"] },
  ...
  { id: "SUP-029", category: "SUP", severity: "BLOCK", title: "Malicious Strapi npm packages (Redis RCE / Credential Harvesting)", description: "36 malicious npm packages disguised as Strapi CMS plugins were discovered in April 2026. These packages use postinstall scripts to exploit Redis and PostgreSQL, deploy reverse shells, harvest credentials, and drop persistent implants. Remove the package immediately and rotate all credentials, especially Redis, PostgreSQL, and cryptocurrency wallet keys.", tags: ["npm", "strapi", "redis", "rce", "credential-harvesting"] },
  { id: "SUP-030", category: "SUP", severity: "BLOCK", title: "a11y-mcp SSRF vulnerability (CVE-2026-5323)", description: "a11y-mcp SSRF vulnerability (CVE-2026-5323)", tags: ["supply-chain", "mcp", "ssrf", "cve-2026-5323", "a11y"] },
  ...
  { id: "SUP-033", category: "SUP", severity: "BLOCK", title: "OpenVSX Code Extension Marketplace Zip Slip path traversal (CVE-2026-35454, pre-2.4.2)", description: "CVE-2026-35454 is a Zip Slip path traversal vulnerability in the Open VSX Code Extension Marketplace prior to version 2.4.2. Maliciously crafted .vsix extension archives can bypass pre-publication security checks by embedding path traversal sequences in archive entry names. When the marketplace extracts such an archive, files are written outside the intended extraction directory, enabling arbitrary file write on the server. Upgrade Open VSX to 2.4.2 or later. Validate all archive entry paths before extraction and reject entries that resolve outside the target directory.", tags: ["supply-chain", "openvsx", "zip-slip", "path-traversal", "cve-2026-35454", "vscode-extension", "arbitrary-file-write"] },
  { id: "MAL-067", category: "MAL", severity: "BLOCK", title: "GlassWorm invisible Unicode PUA payload injection (eval via FE00-FE0F/E0100-E01EF encoding)", description: "A new GlassWorm campaign (Aikido Security, March 2026) hides malicious JavaScript payloads inside invisible Unicode Private Use Area (PUA) characters in npm packages, GitHub repositories, and VS Code extensions. The attacker encodes a complete second-stage payload as invisible Unicode characters in the range U+FE00-U+FE0F and U+E0100-U+E01EF. A decoder function extracts the payload using codePointAt() and executes it via eval(Buffer.from(s(``)).toString()). Audit all npm packages and VS Code extensions for invisible Unicode characters near dynamic execution sinks. Normalize Unicode before processing skill/extension source files.", tags: ["malware", "glassworm", "unicode-pua", "eval", "npm", "vscode-extension", "supply-chain", "steganography", "invisible-payload"] },
  { id: "PSV-012", category: "PSV", severity: "BLOCK", title: "aws-mcp-server command injection RCE (CVE-2026-5058 / CVE-2026-5059, CVSS 9.8)", description: "CVE-2026-5058 and CVE-2026-5059 (ZDI-26-246/ZDI-26-247, GHSA-fjwc-hc62-p8h9, CVSS 9.8) are critical unauthenticated remote code execution vulnerabilities in aws-mcp-server. User-supplied strings in the allowed-commands list are passed to system calls without sanitization, enabling command injection. An unauthenticated attacker can craft a malicious command string to achieve full RCE on the host running the MCP server. Apply vendor patches immediately. Until patched, restrict aws-mcp-server to trusted networks and validate all command arguments against a strict allowlist.", tags: ["mcp", "rce", "command-injection", "aws", "cve-2026-5058", "cve-2026-5059", "zdi-26-246", "zdi-26-247", "unauthenticated", "critical"] },
